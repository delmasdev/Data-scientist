...
def hmany_journalist
	puts "\n \n"
	handle = stock_handle
	puts "> Il y a #{handle.length} journalistes dans ce array !"
	puts "\n \n"
...
def journa_w_nb
	handle = stock_handle
	puts "> Il y a #{handle.grep(/[0-9]/).count} qui contiennent un numéro !"
	puts "\n \n"

end

def journa_4_letters
	handle = stock_handle
	puts "> Il y a #{handle.grep(/aude/i).count} pseudos qui contiennent les 4 lettres du prénom (Aude) à la suite !"
	puts "\n \n"
...
def journa_w_maj
	handle = stock_handle
	puts "> Il y a #{handle.grep(/@\p{Upper}/).count} pseudos avec une majuscule juste après le (@)"
	puts "\n \n"
...
def journa_w_onemaj
	handle = stock_handle
	puts "> Il y a #{handle.grep(/\p{Upper}/).count} pseudos avec une majuscule !"
	puts "\n \n"
...
def journa_nb_under
	handle = stock_handle
	puts "> Il y a #{handle.grep(/_/).count} (_) dans tous les pseudos !"
	puts "\n \n"
...
def list
	...
	print handle.sort_by!{ |handle| handle }
	puts "\n \n"
...
def short
	...
	print handle.min(50){ |a, b| a.length <=> b.length}
	puts "\n \n"
end

def position
	handle = stock_handle
	puts "> La position de la personne @epenser dans le tableau est #{handle.index("@epenser")} \n"
	puts "\n \n"
end
